...
const int COPLANAR_OPPOSITE = -2;
const int COPLANAR_ALIGNED = 2;

const int INVERT_TRI = 0;
const int ADD_TRI = 1;
const int SKIP_TRI = 2;

double FLOATING_COPLANAR_EPSILON = 1e-14;

dynamic _debugContext = null;
    ...
    intersectsTriangles(triangleA, triangleB, ia, ib) {
      if (!isTriDegenerate(triangleA) && !isTriDegenerate(triangleB)) {
        ...
        if (!intersected) {
          ...
          if (na.dot(nb) == 1 && (pa.constant - pb.constant).abs() < FLOATING_COPLANAR_EPSILON) {
            intersected = true;
          }
        }
...
void appendAttributeFromTriangle(int triIndex, Triangle baryCoordTri, geometry, Matrix4 matrixWorld,
  ...
  int i3 = triIndex * 3;
  int i0 = indexAttr.getX(i3 + 0);
  int i1 = indexAttr.getX(i3 + 1);
  int i2 = indexAttr.getX(i3 + 2);
